...
class function TRickDialogProFMX.New(
  const ATheme: IRickDialogProTheme): IRickDialogPro;
begin
  Result := Self.Create(ATheme);
end;

function TRickDialogProFMX.Execute(
  const AConfig: TRickDialogProConfig): TRickDialogProResult;
var
  LForm: TRickDialogProRuntimeForm; // Formulário modal criado para esta execução.
begin
  LForm := nil;
  try
...
function TRickDialogProFMX.Warning(const ATitle, AMessageText,
...
function TRickDialogProFMX.Success(const ATitle, AMessageText,
  APrimaryCaption, ASecondaryCaption: string): TRickDialogProResult;
begin
